...
# 用户点击通知"跳过"按钮时调用
# 写入 response.json（action=skip），然后关闭通知
#
set -e
...
# 立即打印调试日志
echo "$(date): skip script started NONCE=$NONCE TASK_ID=$TASK_ID" >> /tmp/action_debug.log
...
if [ -f "$PENDING_FILE" ] && [ "$(cat "$PENDING_FILE")" != "[]" ]; then
    ...
    if [ -n "$TASK" ] && [ "$TASK" != "null" ]; then
        ...
        echo "$(date): skip script called but task_id=$TASK_ID not found in pending" >> /tmp/action_debug.log
    fi
else
    echo "$(date): skip script called but pending empty or missing" >> /tmp/action_debug.log
fi
