...
NOTIFICATION_DAEMON="$HOME/storage/shared/MyObsidianVaults/scripts_db/FragmentsSkill/OnPhone_task_manage/scripts/review_notification_daemon.sh"
NOTIFICATION_DAEMON_PID="$HOME/storage/shared/MyObsidianVaults/scripts_db/FragmentsSkill/OnPhone_task_manage/docs/notification_daemon.pid"
...
#         rm -f "$TMP_DUE"
        ...
        # 提取到期任务
        jq --argjson idx "$DUE_COUNT" '.[0:$idx]' "$TASK_FILE" > "$TMP_DUE"
        # 立即从原 task.json 中移除这些任务，防止重复提取
        TMP_REMAIN=$(mktemp)
        jq --argjson idx "$DUE_COUNT" '.[$idx:]' "$TASK_FILE" > "$TMP_REMAIN"
        mv "$TMP_REMAIN" "$TASK_FILE"

        # 生成 UUID 并写入 pending (此处逻辑保留你原有的 python 块)
        python3 - << PYEOF
import json
import sys
import uuid
...
PYEOF
        ...
        # 步骤3：确保通知服务在运行
        # ==================================================
        if [ -f "$NOTIFICATION_DAEMON_PID" ]; then
            OLD_DAEMON_PID=$(cat "$NOTIFICATION_DAEMON_PID")
            if ! kill -0 "$OLD_DAEMON_PID" 2>/dev/null; then
                echo "通知服务未运行，正在启动..."
                nohup bash "$NOTIFICATION_DAEMON" >>"$HOME/storage/shared/MyObsidianVaults/scripts_db/FragmentsSkill/OnPhone_task_manage/docs/daemon.log" 2>&1 &
                sleep 1
            else
                echo "通知服务已在运行 (PID $OLD_DAEMON_PID)"
            fi
        else
            echo "通知服务未运行，正在启动..."
            nohup bash "$NOTIFICATION_DAEMON" >>"$HOME/storage/shared/MyObsidianVaults/scripts_db/FragmentsSkill/OnPhone_task_manage/docs/daemon.log" 2>&1 &
        ...
        # ==================================================
        # 步骤4 & 5： 监听并处理，直到 pending 为空
        # ==================================================
        echo "⏳ 正在等待用户操作..."
        
        while [ "$(jq '. | length' "$REVIEW_PENDING_FILE" 2>/dev/null || echo 0)" -gt 0 ]; do
            
            if ! inotifywait -q -t 30 -e close_write "$REVIEW_RESPONSE_FILE" 2>/dev/null; then
                continue
            fi

            # 读取并立刻清空响应，防止处理期间有新响应写入导致丢失
            RESPONSES=$(cat "$REVIEW_RESPONSE_FILE")
            echo "[]" > "$REVIEW_RESPONSE_FILE"
            
            RESPONSE_COUNT=$(echo "$RESPONSES" | jq '. | length' 2>/dev/null || echo 0)

            if [ "$RESPONSE_COUNT" -gt 0 ]; then
                for (( r=0; r<$RESPONSE_COUNT; r++ )); do
                    ITEM=$(echo "$RESPONSES" | jq ".[$r]")
                    RESP_ACTION=$(echo "$ITEM" | jq -r '.action')
                    RESP_PATH=$(echo "$ITEM" | jq -r '.path')
                    RESP_TAG=$(echo "$ITEM" | jq -r '.tag_content')
                    RESP_ORIG_ACTION=$(echo "$ITEM" | jq -r '.action_script')

                    echo "  处理响应: [$RESP_ACTION] $RESP_PATH"
                    ...
                    if [ "$RESP_ACTION" = "complete" ]; then
                        ...
                        # ================= 修复后的追加逻辑 =================
                        echo "  ⏭️ 用户跳过，存入 $ABORT_FILE"
                        
                        # 1. 确保 abort 文件存在
                        if [ ! -f "$ABORT_FILE" ] || [ ! -s "$ABORT_FILE" ]; then echo "[]" > "$ABORT_FILE"; fi
                        
                        # 2. 预清理对象：去掉 task_id, action 等临时字段
                        CLEAN_ITEM=$(echo "$ITEM" | jq 'del(.task_id, .action, .timestamp, .response)')
                        
                        # 3. 使用 --argjson 安全地追加到数组末尾
                        TMP_MERGE=$(mktemp)
                        if jq --argjson newItem "$CLEAN_ITEM" '. += [$newItem]' "$ABORT_FILE" > "$TMP_MERGE"; then
                            mv "$TMP_MERGE" "$ABORT_FILE"
                        else
                            echo "❌ 写入 $ABORT_FILE 失败 (JQ 语法错误)"
                            rm -f "$TMP_MERGE"
                        fi
                        # ==================================================
                    fi
